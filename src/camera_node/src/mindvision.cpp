...
MindVision::MindVision(std::string mindvision_config, const std::string sn)
  ...
  if (i_status != CAMERA_STATUS_SUCCESS) {
    ...
    while ((reinit_times--) != 0) {
      i_status =
          CameraInit(&t_camera_enum_list[camera_index], -1, -1, &h_camera);
      if (i_status == CAMERA_STATUS_SUCCESS) {
        break;
      } else {
        RCLCPP_WARN(logger, "Camera Init failed.");
  ...
  // 创建 rgb 图缓存区
  g_p_rgb_buffer =
      (unsigned char *)malloc(t_capability.sResolutionRange.iHeightMax *
                              t_capability.sResolutionRange.iWidthMax * 3);
